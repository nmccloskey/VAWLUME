...
The three tracking tables are **metadata only**. Unlike the event and anchor
tables, which hold one row per source row, they hold one row per stream, one per
`(entity, bodypart)` trace, and one per mapped column role. No tracking sample
ever enters the IR, whatever the artifact's height. See
[`24_tracking_input_contract.md`](24_tracking_input_contract.md).

Adding them did not bump `ir_schema_version`, following the precedent set when
the alignment work added five tables without one: the IR contract version marks
the shape's generation, and the profile language version (`0.3-draft`) carries
the additive distinction between profile kinds.
