...
fprintf(['All clocks, transforms, events, and coverage below are synthetic and ' ...
    'illustrative.\nNo threshold here is calibrated against real acquisition ' ...
    'hardware.\n']);

fprintf('\nSession and clock scope:\n');
fprintf('  reference timebase   %s (id %d)\n', ...
    value.qc.reference_timebase.timebase_key, ...
    value.qc.reference_timebase.timebase_id);
fprintf('  source timebases     %s\n', ...
    strjoin(value.qc.transforms.source_timebase_key', ', '));
fprintf('  alignment set / run  %d / %s\n', ...
    value.qc.alignment_set_id, value.qc.run_key);
fprintf('  logical anchors      %d in %d observations\n', ...
    value.intake.anchor_summary.anchor_count, ...
    value.intake.anchor_summary.observation_count);

fprintf('\nManifest and mapping-profile provenance:\n');
fprintf('  manifest             %s  sha256 %s\n', ...
    value.intake.manifest.content_uri, ...
    extractBefore(value.intake.manifest.checksum_sha256 + "000000000000", 13));
for index = 1:height(value.intake.profiles)
    fprintf('  profile              %s  v%s  sha256 %s\n', ...
        value.intake.profiles.profile_key(index), ...
        value.intake.profiles.version_label(index), ...
        value.intake.profiles.checksum(index));
end

fprintf('\nRecovered transform parameters:\n');
...
specification = value.regularization_specification;
fprintf('Regularization specification:\n');
fprintf('  window %g to %g s on %s, bin width %g s, origin %g\n', ...
    specification.window_start, specification.window_end, ...
    specification.reference_timebase_key, specification.bin_width, ...
    specification.bin_origin);
fprintf('  edges %s, coverage rule %s, unavailable as %s, persistence %s\n', ...
    specification.edge_convention, specification.coverage_rule, ...
    specification.unavailable_representation, specification.persistence);
