function id = trackingInsertRow(conn, tableName, values, idColumn)
%TRACKINGINSERTROW Insert one scalar structured row and return its SQLite ID.
%
% Empty optional text is omitted so the column stores NULL rather than '',
% matching insertIntakeRow in the ingest layer. Every read in this package
% therefore wraps nullable text in IFNULL: the Database Toolbox raises
% "Unexpected NULL" while building a result set, before MATLAB-side
