...
[plan.candidates, plan.unmatched] = matchingGenerateCandidates( ...
    runA, runB, plan.run_pair, plan.specification.plausibility_rule);
...
% below could map a NULL to "". detections.native_event_id is nullable: the
% column is optional in an extractor output profile, and insertIntakeRow omits
% an empty value rather than writing '', so a profile that does not map one - or
% maps one and does not declare native_event_id_uniqueness at error severity -
% stores NULL. Every shipped profile does declare it, so this is reachable
% through a user-supplied profile rather than the shipped path.
%
% Matching tolerates the absence rather than refusing it. Every decision here is
% keyed on detection_id; native_event_id is carried for reporting only. The
% agreement layer makes the opposite choice, because there the same value forms
% component identity - see agreementResolveGraph.
geometry = fetch(conn, "SELECT detection_id, " + ...
    "IFNULL(native_event_id,'') AS native_event_id, start_time_s, " + ...
...
% details_json is nullable and is compared through presentText below, so it is
% selected through IFNULL for the same reason as detectionGeometry above.
% Candidate generation always writes the column, so only a hand-authored or
% externally inserted candidate row reaches this guard.
