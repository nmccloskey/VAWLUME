function location = extractorPortableLocation(path, declaredRelativePath, roots, errorToken)
%EXTRACTORPORTABLELOCATION Derive durable root-independent artifact identity.
...
if strlength(declaredRelativePath) > 0
    location.relative_path = normalizeDeclaredPath(declaredRelativePath, errorToken);
...
if isempty(parts)
    error(identifier, "Declared artifact identity must name a portable relative path.");
end
path = strjoin(parts, "/");
end
