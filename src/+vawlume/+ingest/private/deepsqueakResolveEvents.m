...
% Both native class columns are nullable and are normalized by presentText
% below, so they are selected through IFNULL alongside canonical_class_label.
% A class written from an empty native label stores NULL rather than '',
% because insertIntakeRow omits empty optional text.
rows = fetch(conn, ...
    "SELECT IFNULL(cc.native_class_id, '') AS native_class_id, " + ...
    "IFNULL(cc.native_class_label, '') AS native_class_label, " + ...
