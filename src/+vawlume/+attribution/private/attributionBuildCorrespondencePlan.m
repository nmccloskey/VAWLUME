...
if options.Apply
    assertNotAlreadyCorresponded(conn, run);
end
...
plan.extent_bases = extentBasesPresent(targets);
end

% ------------------------------------------------------------ re-running ---

function assertNotAlreadyCorresponded(conn, run)
%ASSERTNOTALREADYCORRESPONDED Refuse a second apply, by name.
%
% What a second apply did before this guard, observed rather than assumed: the
% first correspondence INSERT hit
% UNIQUE(imported_attribution_window_id, attribution_target_id), the Database
% Toolbox raised `database:sqlite:interfaceError`, and the transaction rolled
% back leaving the run untouched. So nothing was ever duplicated -- the 4.9
% ledger's "applying twice appends duplicate correspondences" was wrong about
% this table.
%
% The real defect is that the caller could not tell "already applied" from a
% genuine database fault, both arriving as one opaque interface error. And the
% protection was incidental: attribution_evidence has no natural key, so any
% future path writing evidence outside that failing transaction would append
% duplicates with nothing to stop it.
%
% Refusing by name is the imported path's answer to the same question
% (`ImportAlreadyApplied`), for the same reason.
%
% Guarded on Apply only, which is where this layer differs from intake
% deliberately: planning writes nothing and stays useful afterwards -- previewing
% what a different clock declaration would have produced is a legitimate thing to
% do on a run that has already been applied.
existing = fetch(conn, "SELECT COUNT(*) AS n " + ...
    ...
    "WHERE iw.attribution_run_id=" + string(run.attribution_run_id));
count = double(existing.n(1));
if count == 0
    return
end
stored = fetch(conn, "SELECT DISTINCT c.iou_basis AS basis " + ...
    "FROM attribution_window_correspondences c " + ...
    "JOIN imported_attribution_windows iw " + ...
    "  ON iw.imported_attribution_window_id = c.imported_attribution_window_id " + ...
    "WHERE iw.attribution_run_id=" + string(run.attribution_run_id) + ...
    " ORDER BY c.iou_basis");
error("vawlume:attribution:CorrespondenceAlreadyApplied", ...
    "Run %s already carries %d correspondences, on the %s basis. Evidence is " + ...
    "append-only, so a second apply would duplicate rather than reconcile. " + ...
    "Planning still works on this run; to correspond again under different " + ...
    "terms, create a new run.", run.run_key, count, ...
    strjoin(presentText(stored.basis), " and "));
end

function value = extentBasesPresent(targets)
%EXTENTBASESPRESENT Which derived extents this run's targets rest on.
%
% Reported so a caller can see at a glance that one run spans several bases, and
% so results computed on different ones are not pooled by accident. Empty for a
% detection or consensus-event run, which has no extent choice to make.
value = strings(0, 1);
if height(targets) == 0
    return
end
bases = presentText(targets.target_extent_basis);
value = unique(bases(strlength(bases) > 0));
end
...
row = table( ...
    ...
    string(target.target_extent_basis), ...
