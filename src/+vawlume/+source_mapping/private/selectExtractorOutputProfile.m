function [entry, location] = selectExtractorOutputProfile(profileInput, profileId)
%SELECTEXTRACTOROUTPUTPROFILE Resolve a validated extractor-output profile.

profileId = string(profileId);
[documents, locations] = profileDocuments(profileInput);
if isempty(documents)
    error("vawlume:source_mapping:InvalidProfileInput", ...
        "Expected a loaded profile bundle or profile document with a profile envelope.");
end
...
if strlength(profileId) > 0
    ...
    if selectedIndex == 0
        error("vawlume:source_mapping:ProfileIdNotFound", ...
            "Extractor-output profile id was not found: %s", profileId);
    end
...
if kind ~= "extractor_output"
    error("vawlume:source_mapping:UnexpectedProfileKind", ...
        "Expected extractor_output profile but found %s.", kind);
end
end
