...
% native_event_id is selected through IFNULL because MATLAB's Database Toolbox
% raises "Unexpected NULL" while building the result set, before presentText
% below could map a NULL to "". The column is nullable, and an extractor output
% profile that does not map an identifier - or does not declare
% native_event_id_uniqueness at error severity - stores NULL. The guard below
% then refuses that population explicitly rather than composing it.
runList = strjoin(string(extractorRuns.extraction_run_id'), ",");
rows = fetch(conn, "SELECT vc.detection_id, vc.extraction_run_id, " + ...
    "vc.extraction_run_key, vc.extractor_name, " + ...
    "IFNULL(vc.native_event_id,'') AS native_event_id, " + ...
...
% The selector is component identity, not a label. Phase 1.8 made a component's
% group_key the sorted list of its members' selectors, so two detections that
% share a selector produce a group_key that does not distinguish its own
% members, and a rerun cannot recognize what it composed before.
%
% An absent native identifier is therefore refused here rather than tolerated.
% Substituting detection_id would make component identity depend on SQLite
% rowids, and fabricating one from row order is what the import layer already
% refuses to do. Matching makes the opposite choice for the same column because
% it keys on detection_id and carries the identifier for reporting only.
missing = strlength(nodes.native_event_id) == 0;
if any(missing)
    error("vawlume:agreement:NativeEventIdMissing", ...
        "%d detection(s) of extraction run(s) %s carry no native event " + ...
        "identifier, which arbitrary-N agreement uses as component identity. " + ...
        "Import them under a profile that maps one and declares " + ...
        "native_event_id_uniqueness at error severity.", nnz(missing), ...
        strjoin("'" + unique(nodes.extraction_run_key(missing))' + "'", ", "));
end
...
if numel(unique(nodes.selector)) ~= height(nodes)
    [distinct, ~, grouping] = unique(nodes.selector);
    repeated = distinct(accumarray(grouping, 1) > 1);
    error("vawlume:agreement:NodeSelectorAmbiguous", ...
        "Selector %s identifies more than one detection. A native event " + ...
        "identifier must be unique within its extraction run.", ...
        strjoin("'" + repeated(:)' + "'", ", "));
end
end
