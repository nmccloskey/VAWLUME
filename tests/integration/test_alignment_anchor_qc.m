...
tests = functiontests({ ...
    ...
    @testStructuredObservationSelectorIsReadableAndRefusesAmbiguity, ...
...
function testStructuredObservationSelectorIsReadableAndRefusesAmbiguity(testCase)
[fixture, cleanup] = setUpFixture(); %#ok<ASGLU>

selector = alignmentRef();
selector.anchor_key = "sync02";
selector.timebase_key = "audio_native";
selector.observation_role = "primary";
result = vawlume.alignment.setAnchorInclusion(fixture.conn, selector, false, ...
    Reason="synthetic structured-selector check");
verifyEqual(testCase, result.anchor_observation_id, ...
    audioObservationId(fixture, "sync02"));
verifyEqual(testCase, result.action, "withheld");

addReplicate(fixture, "sync01", fixture.audio_timebase_id, ...
    audioTime(fixture, "sync01") + 0.006);
ambiguous = alignmentRef();
ambiguous.anchor_key = "sync01";
ambiguous.timebase_key = "audio_native";
verifyError(testCase, @() vawlume.alignment.setAnchorInclusion( ...
    fixture.conn, ambiguous, false, Reason="must select one reading"), ...
    "vawlume:alignment:AnchorObservationAmbiguous");
verifyError(testCase, @() vawlume.alignment.setAnchorInclusion( ...
    fixture.conn, struct(anchor_key="sync01"), false, Reason="malformed"), ...
    "vawlume:alignment:AnchorObservationRefInvalid");
