...
execute(fixture.conn, "INSERT INTO alignment_anchor_observations(" + ...
    ...
    "observation_role, included_in_fit, uncertainty_s, uncertainty_semantics, " + ...
    "notes) VALUES (" + ...
    string(anchorId(fixture, "sync02")) + ", " + string(fixture.audio_timebase_id) + ...
    ", " + string(audioTime(fixture, "sync02") + 0.04) + ...
    ", 'replicate', 0, 0.004, 'declared_anchor_reading_uncertainty_s', " + ...
    "'Redundant TTL channel.')");
...
function testApplyRefusesUnfittedRejectedAndUntiledRuns(testCase)
[fixture, cleanup] = setUpFixture(); %#ok<ASGLU>
...
% A second segment bounded away from the first leaves source times with no
% segment at all. Piecewise transforms are applied now, but only when their
% segments tile: a gap would silently produce a plausible-looking wrong answer
% for every time that falls in it. The write guard cannot see a database edited
% by hand, so the applier checks on read as well.
