...
execute(fixture.conn, ...
    "UPDATE curation_events SET status_after = 'rejected' WHERE detection_id = " + ...
    "(SELECT detection_id FROM detections WHERE native_event_id = '2')");
execute(fixture.conn, ...
    "UPDATE curation_events SET details_json = '{}' WHERE detection_id = " + ...
    "(SELECT detection_id FROM detections WHERE native_event_id = '3')");
execute(fixture.conn, ...
    "UPDATE classification_assignments SET classification_class_id = (" + ...
    "SELECT cc.classification_class_id FROM classification_classes cc " + ...
    "WHERE cc.native_class_id = 'USV') WHERE detection_id = " + ...
    "(SELECT detection_id FROM detections WHERE native_event_id = '3')");
execute(fixture.conn, ...
    "UPDATE classification_assignments SET assignment_source = 'human' " + ...
    "WHERE detection_id = (SELECT detection_id FROM detections WHERE native_event_id = '2')");
...
verifyTrue(testCase, any(contains(conflicted.conflicts, "stored review status differs")));
verifyTrue(testCase, any(contains(conflicted.conflicts, "native review token or field")));
verifyTrue(testCase, any(contains(conflicted.conflicts, "stored native label differs")));
verifyTrue(testCase, any(contains(conflicted.conflicts, "assignment source or score")));
...
verifyEqual(testCase, conflicted.detections.curation_conflict, 2);
verifyEqual(testCase, ...
    conflicted.detections.classification_assignments_conflict, 2);
