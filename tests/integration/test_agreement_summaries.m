...
% The naive approach: pair features whose canonical names are equal. With three
% registered extractors it fails in both directions at once.
...
verifyEqual(testCase, numel(names), 9);

% False negative: DeepSqueak's central-frequency measure is named
% contour_median_frequency, so no canonical-name join can reach the
% DeepSqueak-MUPET central-frequency pair that the registry does register.
verifyFalse(testCase, ismember("contour_median_frequency", names));
verifyEqual(testCase, height(fetch(fixture.conn, ...
    "SELECT cf.canonical_name FROM extractor_features xa " + ...
    "JOIN feature_mappings fma ON fma.extractor_feature_id = xa.extractor_feature_id " + ...
    "JOIN canonical_features cf ON cf.canonical_feature_id = fma.canonical_feature_id " + ...
    "JOIN feature_mappings fmb ON fmb.canonical_feature_id = cf.canonical_feature_id " + ...
    "JOIN extractor_features xb ON xb.extractor_feature_id = fmb.extractor_feature_id " + ...
    ...
    "WHERE ea.extractor_name = 'DeepSqueak' AND eb.extractor_name = 'MUPET' " + ...
    "AND cf.canonical_name IN ('frequency_center', 'contour_median_frequency')")), 0);

% False positive: a shared canonical name now also proposes pairs the registry
% deliberately refuses. USVSEG shares peak_frequency with DeepSqueak and
% peak_amplitude with MUPET, and neither pair is comparable.
for invented = ["peak_frequency", "peak_amplitude"]
    verifyTrue(testCase, ismember(invented, names), invented);
end
verifyEqual(testCase, height(fetch(fixture.conn, ...
    "SELECT fr.feature_relationship_id FROM feature_relationships fr " + ...
    "JOIN extractor_features fa ON fa.extractor_feature_id = fr.feature_a_id " + ...
    "JOIN feature_mappings fma ON fma.extractor_feature_id = fa.extractor_feature_id " + ...
    "JOIN canonical_features ca ON ca.canonical_feature_id = fma.canonical_feature_id " + ...
    "JOIN extractor_features fb ON fb.extractor_feature_id = fr.feature_b_id " + ...
    "JOIN feature_mappings fmb ON fmb.extractor_feature_id = fb.extractor_feature_id " + ...
    "JOIN canonical_features cb ON cb.canonical_feature_id = fmb.canonical_feature_id " + ...
    "WHERE fr.consilience_eligible = 1 " + ...
    "AND ca.canonical_name = cb.canonical_name " + ...
    "AND ca.canonical_name IN ('peak_frequency', 'peak_amplitude')")), 0);
